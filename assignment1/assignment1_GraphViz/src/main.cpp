...
void forceDirectedLayout(SimpleGraph& sg){
    ...
    for(int i=0;i<n;i++){ // repel force between each pair of nodes.
        for(int j=i+1;j<n;j++){
            ...
            forces[i].Fx -= forceRepel*cos(theta);
            forces[i].Fy -= forceRepel*sin(theta);
            forces[j].Fx += forceRepel*cos(theta);
            forces[j].Fy += forceRepel*sin(theta);
        }
    }
    ...
    for(int i=0;i<sum;i++){// attractive force between Edge start node & Edge end node.
        ...
        forces[sg.edges[i].start].Fx += forceAtt*cos(theta);
        forces[sg.edges[i].start].Fy += forceAtt*sin(theta);
        forces[sg.edges[i].end].Fx -= forceAtt*cos(theta);
        forces[sg.edges[i].end].Fy -= forceAtt*sin(theta);
    }

    for(int i=0;i<n;i++){// apply force to each node.


        sg.nodes[i].x+=forces[i].Fx;

        sg.nodes[i].x+=randomFloat(0,0.01);
        sg.nodes[i].y+=forces[i].Fy;
        sg.nodes[i].y+=randomFloat(0,0.01);
    }
